...
class NodeApi:
    ...
    def connect_peer(
        ...
        addrs: Annotated[Optional[List[StrictStr]], Field(description="If supplied, it will be used to dial the peer. The address has to target the listening address of the peer, which is /ip4/<listen-ip>/tcp/<listen-port>, where `listen-port` is specified with the `--listen-port` CLI flag and `listen-ip` with the `--listen-ip` CLI flag. ")] = None,
        ...
        :type peer_id: str
        :param addrs: If supplied, it will be used to dial the peer. The address has to target the listening address of the peer, which is /ip4/<listen-ip>/tcp/<listen-port>, where `listen-port` is specified with the `--listen-port` CLI flag and `listen-ip` with the `--listen-ip` CLI flag. 
        :type addrs: List[str]
